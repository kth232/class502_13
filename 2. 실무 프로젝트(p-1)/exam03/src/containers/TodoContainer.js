...
const TodoContainer = () => { //업데이트 시 매번 호출
  ...
  console.log('same function? :', submitFunc === onSubmit);
  submitFunc = onSubmit;
  ...
  const onToggle = useCallback((id) => {
    ...
    setItems(newItems); //최초 생성 시 값이 고정되어버림
    */
   setItems((prevItems)=> {
    return prevItems.map((item) => item.id === id ? {...item, done:!item.done}:item);
   });
  }, []); //함수형 업데이트, 변화감지할 필요없음, 호출할 때마다 값을 넣어줌, 낭비x
